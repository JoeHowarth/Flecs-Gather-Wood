...
void transfer_items(flecs::entity dst, flecs::entity src) {
    ...
    dst.world().defer([&] {
        dst =
            get_container(dst);  // Make sure to replace players with container
...
void print_items(flecs::entity container) {
    ...
    for_each_item(container, [&](flecs::entity item) {
        ...
        int32_t amount = 1;
        item.get([&](const Amount &amt) { amount = amt.value; });
...
int foo(int, char *[]) {
    ...
    ecs.prefab<WoodenSword>()
        .add<Sword>()
        .set<Attack>({1})
        .set_override<Health>({5});  // copy to instance, don't share

    ecs.prefab<IronSword>().add<Sword>().set<Attack>({2}).set_override<Health>(
        {10}
    );

    ecs.prefab<WoodenArmor>().add<Armor>().set_override<Health>({10});

    ecs.prefab<IronArmor>().add<Armor>().set_override<Health>({20});
    ...
    flecs::entity player =
        ecs.entity("Player").set<Health>({10}).add<Inventory>(
    ...
    flecs::entity my_sword = ecs.entity().is_a<IronSword>();
